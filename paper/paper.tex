...
\begin{abstract}
\texttt{DistributedWorkflows.jl} is a serializer-independent interface to a distributed task-based workflow management system.
...
This makes \texttt{DistributedWorkflows.jl} an invaluable addition to Julia's growing ecosystem of high-performance computing.
\end{abstract}
...
In this paper, we introduce \texttt{DistributedWorkflows.jl} \cite{DistributedWorkflows}, a Julia package that aims to facilitate the writing of parallel applications. 
\texttt{DistributedWorkflows.jl} is a high-level wrapper around the HPC tool GPI-Space \cite{GSPC}, which allows domain scientists to create efficient parallel applications with a user-friendly interface. 
...
Section \ref{sec:how_to_use} provides a short description of how to use \texttt{DistributedWorkflows.jl}, while Section \ref{sec:limitations} discusses its limitations. 
...
The most popular of these packages are \texttt{Distributed.jl} \cite{Distributedjl} and \texttt{Dagger.jl} \cite{dagger1, dagger2}.\vskip 6pt

\texttt{Distributed.jl} offers functionalities for creating and managing multiple Julia processes remotely, enabling distributed and parallel computing. 
...
\texttt{Dagger.jl} is a scheduler that draws inspiration from Dask \cite{dask}. 
It efficiently executes computations represented as directed-acyclic graphs (DAGs) across multiple Julia worker processes and threads, as well as GPUs via \texttt{DaggerGPU.jl}. 
...
\texttt{DistributedWorkflows.jl} uses GPI-Space \cite{GSPC} as its underlying framework.
...
This makes Petri nets, as used in \texttt{DistributedWorkflows.jl}, a more powerful model for designing distributed applications, as they allow for greater expressiveness and ease in handling complex tasks and synchronization.
Table \ref{comparison table for distributed, dagger, and distributedworkflows} provides a feature comparison between \texttt{Distributed.jl}, \texttt{Dagger.jl}, and \texttt{DistributedWorkflows.jl}.\vskip 6pt

This package was created out of the need to provide a convenient way to parallelize computations in the Julia package OSCAR \cite{OSCAR}. OSCAR is a computer algebra system that combines the capabilities of its four cornerstones GAP \cite{GAP4}, \texttt{Polymake} \cite{Polymake}, ANTIC % \cite{Antic} 
and \texttt{Singular} \cite{DGPS_Singular}. Each of these cornerstones come equipped with their own serializer, in addition to OSCAR's own serializer that works from within Julia. Hence, the need of \texttt{DistributedWorkflows.jl} to be serializer independent.\vskip 6pt

\texttt{Singular} already employs GPI-Space to write parallel applications, for example see \cite{gpispace_paper_singular}. 
However, \texttt{Singular}’s workflows are custom-designed for specific applications and are not reusable across different tasks. 
...
Although \texttt{Singular} has been using GPI-Space for some time, it lacks a user-friendly interface. 
Furthermore, \texttt{Singular}’s serializer is fixed to its custom \verb+ssi+ format.\vskip 6pt

In contrast, Julia’s flexibility allows users to bring their own serializer, which makes \texttt{DistributedWorkflows.jl} a more versatile package in terms of serialization options.\vskip 6pt
...
In this section, we provide a short background on Petri nets used to define workflows in \texttt{DistributedWorkflows.jl} and its underlying distributed execution framework GPI-Space.

\subsection{Petri Nets}
\label{sec:petri_nets}

The programming model of \texttt{DistributedWorkflows.jl} uses Petri nets to define workflow patterns, which enable the modeling of concurrent and distributed systems.\vskip 6pt
...
\texttt{DistributedWorkflows.jl} uses high-level, timed, and colored Petri nets to be consistent with GPI-Space, our underlying system.
...
The \texttt{DistributedWorkflows.jl} package started as an interface to GPI-Space \cite{GSPC}, but it has grown to include features that significantly improves user experience.
...
\begin{figure}[h]
 ...
 width=\linewidth, keepaspectratio]{pluto_example.jpeg}}
    ...
    \caption{Snippet of a simple Petri net workflow created and viewed using a \texttt{Jupyter} notebook in (a), and a \texttt{Pluto} notebook in (b).}
\end{figure}
...
\texttt{DistributedWorkflows.jl} on the other hand, provides binaries for selected operating systems and architectures, that can be downloaded from the GitHub repository \cite{DistributedWorkflows}.
...
Once the setup is complete, the \texttt{DistributedWorkflows.jl} package can be installed directly in Julia using its package manager. 
At this stage, users are ready to parallelize and execute their applications with \texttt{DistributedWorkflows.jl}.\vskip 6pt

Without the interface of \texttt{DistributedWorkflows.jl}, the user will have to embed their parallel Julia algorithm within the workflow which is written in the \verb+XML+ based \verb+XPNet+ format. 
...
The \texttt{DistributedWorkflows.jl} package provides a workflow builder in the form of a Petri net, developed entirely in Julia, which can be visualized and edited without compiling the workflow or having a running code available. See for example Listing \ref{lst:distributed_workflows_pnet_example}.
...
\section{How to Use \texttt{DistributedWorkflows.jl}}
\label{sec:how_to_use}

In this section, we will give a simple example showcasing the use of \texttt{DistributedWorkflows.jl} for parallel applications in the Julia ecosystem.\vskip 6pt
...
\begin{figure}[h]
    ...
    \caption{A Petri net (generated using \texttt{DistributedWorkflows.jl}) modeling the workflow to compute the \emph{Minkowski sum} of a collection of sets of points in $\mathbb{R}^2$.}
    \label{patrinet_example_how_to_use}
\end{figure}

This Petri net can be written in Julia using \texttt{DistributedWorkflows} as shown in Listing \ref{lst:distributed_workflows_pnet}.
...
As a more complex example, a Petri net workflow (see Figure \ref{complex_example}) that models different repair synthesis pathways of DNA as in \cite{biology_example_petrinets_openaccess} can also be tested using \texttt{DistributedWorkflows.jl}.
...
Like any software, the \texttt{DistributedWorkflows.jl} package has certain limitations.
...
This paper and the \texttt{DistributedWorkflows.jl} package introduce a user-friendly interface for managing distributed, task-based workflows. 
...
As well as, enabling her to work on and develop \texttt{DistributedWorkflows.jl}. 
