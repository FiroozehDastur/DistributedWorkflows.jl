...
This package was created out of the need to provide a convenient way to parallelize computations in the Julia package OSCAR \cite{OSCAR}. OSCAR is a computer algebra system that combines the capabilities of its four cornerstones \emph{GAP} \cite{GAP4}, \emph{Polymake} \cite{Polymake}, ANTIC % \cite{Antic} 
and \emph{Singular} \cite{DGPS_Singular}. Each of these cornerstones come equipped with their own serializer, in addition to OSCAR's own serializer that works from within Julia. Hence, the need of \emph{DistributedWorkflows.jl} to be serializer independent.\vskip 6pt
