...
function client_init(workers::Int, nodefile::String, rif_strategy::String, log_host::String, log_port::Int)
  ...
  Client(worker, client_config)
end

function client_init(workers::Int, nodefile::String, rif_strategy::String)
  worker = string("worker:", workers)
  gspc_home = string("--gspc-home=", readchomp(`spack location -i gpi-space`))
  nodefile = string("--nodefile=", nodefile)
  rif_strategy = string("--rif-strategy=", rif_strategy)
